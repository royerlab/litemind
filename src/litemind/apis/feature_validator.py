"""
Model Feature Validator

This module provides the ModelFeatureValidator class for validating and discovering
model feature support through live API calls.

For production feature lookups, use the curated ModelRegistry from model_registry.py
which provides accurate data without requiring API calls.

The ModelFeatureValidator class is used for:
1. Validating registry data against live APIs
2. Discovering features for new/unknown models
3. Generating scan results for manual review
"""
...
class ModelFeatureValidator(MediaResources):
    """
    A class for validating and discovering model feature support through live API calls.

    This class is used for:
    1. Validating curated registry data against live APIs
    2. Discovering features for new/unknown models
    3. Generating scan results for manual review

    For production feature lookups, use ModelRegistry from model_registry.py,
    which provides accurate curated data without requiring API calls.
    """
    ...
    def validate_model(
        ...
        """
        Validate registry data against live API for a specific model.

        This method compares the curated registry data against actual live API
        tests to identify discrepancies.

        Parameters
        ----------
        api_class : Type[BaseApi]
            The API class to validate.
        model_name : str
            The model name to validate.
        features : Optional[List[ModelFeatures]]
            Specific features to validate. If None, validates all features.
        ...
        Dict[str, Dict[str, Union[bool, str]]]
            ...
            {
                feature_name: {
                    "registry": bool,  # Registry value
                    "live": bool,      # Live test result
        ...
        if features is None:
            features = [f for f in ModelFeatures if "conversion" not in f.name.lower()]

        results: Dict[str, Dict[str, Union[bool, str]]] = {}

        with asection(f"Validating {model_name} from {api_class.__name__}"):
            for feature in features:
                registry_value = registry.supports_feature(
                    api_class, model_name, feature
                )
                ...
                match = registry_value == live_value
                icon = "✅" if match else "❌"
                aprint(
                    f"  {icon} {feature.name}: registry={registry_value}, live={live_value}"
                )
    ...
    def discover_features(
        ...
        """
        Discover features for a model not in the registry through live testing.
        ...
        -------
        Dict[ModelFeatures, bool]
            Dictionary mapping features to their discovered support status.
        """
        try:
            api_instance = api_class(allow_media_conversions=False)
            return self.scan_model_features(api_instance, model_name)
        except Exception as e:
            aprint(f"Error discovering features for {model_name}: {e}")
            return {}
    ...
    def validate_apis(
        ...
        model_names: Optional[List[str]] = None,
        features: Optional[List[ModelFeatures]] = None,
    ) -> Dict[Type[BaseApi], Dict[str, Dict[str, Dict[str, Union[bool, str]]]]]:
        """
        Validate registry data against live APIs for multiple APIs/models.

        Parameters
        ----------
        api_classes : List[Type[BaseApi]]
            List of API classes to validate.
        model_names : Optional[List[str]]
            Specific model names to validate. If None, validates models in registry.
        features : Optional[List[ModelFeatures]]
            Specific features to validate. If None, validates all features.
        ...
        -------
        Dict[Type[BaseApi], Dict[str, Dict[str, Dict[str, Union[bool, str]]]]]
            Nested dictionary with validation results per API, model, and feature.
        """
        from litemind.apis.model_registry import get_default_model_registry

        registry = get_default_model_registry()
        all_results: Dict[
            Type[BaseApi], Dict[str, Dict[str, Dict[str, Union[bool, str]]]]
        ] = {}

        for api_class in api_classes:
            all_results[api_class] = {}

            # Get models to validate
            if model_names:
                models_to_validate = model_names
            else:
                models_to_validate = registry.list_models(api_class)

            for model_name in models_to_validate:
                all_results[api_class][model_name] = self.validate_model(
                    api_class, model_name, features
                )
    ...
    def generate_validation_report(
        ...
        """
        md_lines = []
        md_lines.append("# Model Registry Validation Report")
        md_lines.append(f"_Report generated on: {datetime.now().isoformat()}_")
        md_lines.append("")
        ...
        for api_class, models_data in validation_results.items():
            api_name = self._api_class_to_name(api_class)
            md_lines.append(f"## API: {api_name}")
            md_lines.append("")
            ...
            for model_name, features_data in models_data.items():
                model_matches = sum(
                    1 for f in features_data.values() if f.get("match", False)
                )
                model_mismatches = sum(
                    1 for f in features_data.values() if not f.get("match", True)
                )

                api_matches += model_matches
                api_mismatches += model_mismatches

                if model_mismatches > 0:
                    md_lines.append(f"### Model: {model_name}")
                    md_lines.append("")
                    md_lines.append("| Feature | Registry | Live | Status |")
                    md_lines.append("|---------|----------|------|--------|")

                    for feature_name, result in features_data.items():
                        if not result.get("match", True):
                            registry_val = "✅" if result["registry"] else "❌"
                            live_val = "✅" if result["live"] else "❌"
                            md_lines.append(
                                f"| {feature_name} | {registry_val} | {live_val} | MISMATCH |"
                            )

                    md_lines.append("")

            total_tests += api_matches + api_mismatches
            total_matches += api_matches
            total_mismatches += api_mismatches

            match_rate = (
                (api_matches / (api_matches + api_mismatches) * 100)
                if (api_matches + api_mismatches) > 0
                else 100
            )
            md_lines.append(
                f"**{api_name} Summary:** {api_matches} matches, {api_mismatches} mismatches ({match_rate:.1f}% accuracy)"
            )
            md_lines.append("")
            md_lines.append("---")
            md_lines.append("")
        ...
        # Overall summary
        overall_rate = (total_matches / total_tests * 100) if total_tests > 0 else 100
        md_lines.append("## Overall Summary")
        md_lines.append("")
        md_lines.append(f"- **Total Tests:** {total_tests}")
        md_lines.append(f"- **Matches:** {total_matches}")
        md_lines.append(f"- **Mismatches:** {total_mismatches}")
        md_lines.append(f"- **Overall Accuracy:** {overall_rate:.1f}%")
