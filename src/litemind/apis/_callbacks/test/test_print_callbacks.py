...
def test_on_document_conversion(print_callbacks, capsys):
    ...
    captured = capsys.readouterr()
    assert "Document Conversion: Document URI: document_uri, Markdown: markdown" in captured.out
    assert "Additional arguments: {'param': 'value'}" in captured.out

def test_on_video_conversion(print_callbacks, capsys):
    print_callbacks.on_video_conversion("video_uri", ["image1", "image2"], "audio", param="value")
    captured = capsys.readouterr()
    assert "Video Conversion: Video URI: video_uri, Images: ['image1', 'image2'], Audio: audio" in captured.out
    assert "Additional arguments: {'param': 'value'}" in captured.out
...
def test_on_text_embedding(print_callbacks, capsys):
    print_callbacks.on_text_embedding(["text1", "text2"], [[0.1, 0.2], [0.3, 0.4]], param="value")
    captured = capsys.readouterr()
    assert "Text Embedding: Texts: ['text1', 'text2'], Embeddings: [[0.1, 0.2], [0.3, 0.4]]" in captured.out
    assert "Additional arguments: {'param': 'value'}" in captured.out

def test_on_image_embedding(print_callbacks, capsys):
    print_callbacks.on_image_embedding(["image_uri1", "image_uri2"], [[0.1, 0.2], [0.3, 0.4]], param="value")
    captured = capsys.readouterr()
    assert "Image Embedding: Image URIs: ['image_uri1', 'image_uri2'], Embeddings: [[0.1, 0.2], [0.3, 0.4]]" in captured.out
    assert "Additional arguments: {'param': 'value'}" in captured.out

def test_on_audio_embedding(print_callbacks, capsys):
    print_callbacks.on_audio_embedding(["audio_uri1", "audio_uri2"], [[0.1, 0.2], [0.3, 0.4]], param="value")
    captured = capsys.readouterr()
    assert "Audio Embedding: Audio URIs: ['audio_uri1', 'audio_uri2'], Embeddings: [[0.1, 0.2], [0.3, 0.4]]" in captured.out
    assert "Additional arguments: {'param': 'value'}" in captured.out

def test_on_video_embedding(print_callbacks, capsys):
    print_callbacks.on_video_embedding(["video_uri1", "video_uri2"], [[0.1, 0.2], [0.3, 0.4]], param="value")
    captured = capsys.readouterr()
    assert "Video Embedding: Video URIs: ['video_uri1', 'video_uri2'], Embeddings: [[0.1, 0.2], [0.3, 0.4]]" in captured.out
    assert "Additional arguments: {'param': 'value'}" in captured.out
...
def test_on_image_description(print_callbacks, capsys):
    ...
    captured = capsys.readouterr()
    assert "Image Description: Image URI: image_uri, Description: description" in captured.out
    assert "Additional arguments: {'param': 'value'}" in captured.out
...
def test_on_audio_description(print_callbacks, capsys):
    ...
    captured = capsys.readouterr()
    assert "Audio Description: Audio URI: audio_uri, Description: description" in captured.out
    assert "Additional arguments: {'param': 'value'}" in captured.out
...
def test_on_video_description(print_callbacks, capsys):
    ...
    captured = capsys.readouterr()
    assert "Video Description: Video URI: video_uri, Description: description" in captured.out
    assert "Additional arguments: {'param': 'value'}" in captured.out