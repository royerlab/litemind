...
class DummyAugmentation(AugmentationBase):
    ...
    def get_relevant_documents(self, query, k=5):
        return [Document(content=f"Result {i} for {query}", metadata={"score": i}) for i in range(k)]
...
def test_get_augmentation():
    dummy_aug = DummyAugmentation(name="dummy")
    aug_set = AugmentationSet(augmentations=[dummy_aug])
    assert aug_set.get_augmentation("dummy") == dummy_aug
...
def test_list_augmentations():
    ...
    aug_set = AugmentationSet(augmentations=[dummy_aug1, dummy_aug2])
