...
def test_tool_initialization():
    tool = FunctionTool(func=sample_function_one,
                        description="Test function with two parameters")
...
def test_tool_with_no_parameters():
    tool = FunctionTool(func=sample_function_no_params,
    ...
    assert tool.parameters == expected_parameters, "Expected empty parameters for function with no parameters"


# Test parameter schema generation with default values
def test_tool_with_default_parameter():
    tool = FunctionTool(func=sample_function_with_defaults,
    ...
    assert tool.parameters == expected_parameters, "Expected param2 to be optional due to default value"
...
def test_tool_execution_with_parameters():
    ...
    assert result == "Received 42 and example", "Execution result mismatch for sample_function_one"
...
def test_tool_execution_no_parameters():
    ...
    # Execute the tool and check the result
    result = tool.execute()
    assert result == "No parameters here!", "Execution result mismatch for sample_function_no_params"
...
def test_tool_execution_with_default_parameters():
    ...
    assert result_with_default == "Received 100 and default", "Default parameter not applied correctly"

    # Test execution with overridden default parameter
    result_with_override = tool.execute(param1=100, param2="custom")
    assert result_with_override == "Received 100 and custom", "Parameter override not applied correctly"
