...
def test_agent_tool_translation():
    ...
    system_message = Message(role='system', text="You are a translator that translates English sentences to French.")
    ...
    assert len(agent.conversation) == 3, "Agent's conversation should have 3 messages: system, prompt, and assistant's response"
...
def test_agent_tool_with_internal_tool():
    ...
    agent += Message(role='system', text="You are an assistant that provides order status updates.")

    # Initialize the AgentTool with the Agent instance
    agent_tool = AgentTool(agent, "Assistant agent that processes order inquiries and has a tool to check order status")
    ...
    assert len(agent.conversation) == 3, "Agent's conversation should have 3 messages: system, user and assistant's response"
    assert agent.conversation[0].role == 'system'
    assert agent.conversation[1].role == 'user'
    assert agent.conversation[2].role == 'assistant'
