...
def test_react_agent_single_tool():
    ...
    # Get a model that supports text generation:
    model = api.get_best_model([ModelFeatures.TextGeneration])
    ...
    assert (expected_date in response) or (year in response and month in response and day in response)
...
def test_react_agent_longer_dialog():
    ...
    assert (expected_date in response1) or (year in response1 and month in response1 and day in response1)
